...
@st.cache_data
def load_data():
    # 1. 현재 파일(nemo_dashboard.py)의 위치를 기준으로 경로 설정
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 2. 깃허브에 올린 폴더 구조에 맞춰 경로 수정 
    # src 폴더의 상위 폴더(nemostore)로 이동 후 data 폴더 접근
    parent_dir = os.path.dirname(current_dir)
    db_path = os.path.join(parent_dir, "data", "nemostore.db") 
    
    # 만약 경로를 찾지 못할 경우를 대비한 체크
    if not os.path.exists(db_path):
        # 파일이 없을 경우 현재 폴더 및 루트에서 직접 찾아보기 (예비용)
